...
#ifndef EXAMPLES_GRAPHICS_PHOTO_C_SALT_IMAGE_H_
#define EXAMPLES_GRAPHICS_PHOTO_C_SALT_IMAGE_H_
...
class Image {
 public:
  ...
  // is clamped to (1, 1). Use the existing allocated memory if it is large
  // enough.  If the existing pixel buffer is too small, it is replace with
  // a new larger one.  The new pixel buffer is left as raw memory: if a new
  // one is allocated, the old one is not copied, nor is the new buffer cleared.
  // Use Copy() or Erase() to re-fill the new (ore resized) buffer.
  void Resize(int width, int height);

  // Set all image pixels to the current background color.
  void Erase();

  // Load pixels from blob. Only JPEG is supported for now.
...
#endif  // EXAMPLES_GRAPHICS_PHOTO_C_SALT_IMAGE_H_
