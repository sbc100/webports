...
bool ScriptingBridge::AddPropertyNamed(
  ...
  property_accessor_dictionary_.insert(
      std::pair<NPIdentifier, PropertyAccessorCallbackExecutor*>(
          property_id, property_accessor));
  if (property_mutator) {
    property_mutator_dictionary_.insert(
        std::pair<NPIdentifier, PropertyMutatorCallbackExecutor*>(
            property_id, property_mutator));
  }
  return true;
}
...
static bool Invoke(NPObject* object, NPIdentifier name,
                   const NPVariant* args, uint32_t arg_count,
                   NPVariant* result) {
...
NPClass photo::ScriptingBridge::bridge_class = {
