...
/**
 * The Lua-powered terminal command.
 *
 ...
 *
 * This constructs a new Terminal instance and instructs it to run the Lua
 * command.
 */
Lua.init = function() {
  ...
  terminal.runCommandClass(Lua, document.location.hash.substr(1));
 ...
 * @private
 */
Lua.prototype.handleMessage_ = function(e) {
  if (e.data.indexOf(Lua.prefix_) != 0) return;
  var msg = e.data.substring(Lua.prefix_.length);
  term_.io.print(msg);
}
