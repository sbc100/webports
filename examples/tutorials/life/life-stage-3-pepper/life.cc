...
#include "examples/tutorials/life/life-stage-3-pepper/life.h"
...
#include "examples/tutorials/life/life-stage-3-pepper/scoped_mutex_lock.h"
#include "examples/tutorials/life/life-stage-3-pepper/scoped_pixel_lock.h"
...
bool Life::HandleInputEvent(const PP_InputEvent& event) {
  bool add_cell = false;
  switch (event.type) {
  ...
  case PP_INPUTEVENT_TYPE_MOUSEMOVE:
    ...
    break;
  default:
    return false;  // Event not handled, bubble it back up to the browser.
  }
  if (add_cell) {
    float x = event.u.mouse.x;
    float y = event.u.mouse.y;
    AddCellAtPoint(static_cast<int>(x), static_cast<int>(y));
  }
