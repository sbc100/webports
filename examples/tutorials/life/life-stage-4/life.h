...
class Life : public pp::Instance {
 public:
  ...
  // point, the file is available to the module.  Decompress the JPEG data into
 ...
 private:
  ...
  // Resize all the buffers - the pixel buffer containing the final image as
  // well as the cell buffers.  Returns success.
  bool ResizeBuffers(const pp::Size& size);
  ...
  pp::Size background_image_size_;
  // The uncompressed backgorund image in ARGB format.
  boost::scoped_array<uint32_t> background_image_;
